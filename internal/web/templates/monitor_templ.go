...
func configDialog(m store.MonitorRow, cfg MonitorConfig) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		ctx = templ.ClearChildren(ctx)
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 35, "<dialog id=\"monitor-config-dialog\" class=\"w-full max-w-2xl rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 backdrop:bg-slate-900/60 p-0\"><form method=\"dialog\" class=\"flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-4 py-3\"><h2 class=\"text-sm uppercase tracking-wide text-slate-500 dark:text-slate-400\">Effective configuration</h2><button class=\"text-sm px-3 py-1 rounded border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800\" aria-label=\"Close\">Close</button></form><div class=\"px-4 py-3 max-h-[70vh] overflow-y-auto\"><dl>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		templ_7745c5c3_Err = kv("Slug", m.Slug).Render(ctx, templ_7745c5c3_Buffer)
